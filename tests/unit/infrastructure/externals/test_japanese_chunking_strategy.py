...
class TestJapaneseChunkingStrategy:
    ...
    def test_split_text_empty(self, strategy: JapaneseChunkingStrategy) -> None:
        """空のテキストで空のリストが返されることを確認する。"""
        chunks = strategy.split_text("", chunk_size=100, overlap_size=20)
        assert chunks == []

    def test_split_text_with_quotes(self, strategy: JapaneseChunkingStrategy) -> None:
        """引用符が正しく処理されることを確認する。"""
    ...
    def test_split_text_with_overlap(self, strategy: JapaneseChunkingStrategy) -> None:
        """オーバーラップが機能することを確認する。"""
    ...
    def test_estimate_chunk_count(self, strategy: JapaneseChunkingStrategy) -> None:
        """チャンク数の推定が正しいことを確認する。"""
        text = "あ" * 250
    ...
    def test_estimate_chunk_count_single(
        ...
        count = strategy.estimate_chunk_count(
            "短いテキスト", chunk_size=100, overlap_size=20
        )
        assert count == 1

    def test_mixed_content(self, strategy: JapaneseChunkingStrategy) -> None:
